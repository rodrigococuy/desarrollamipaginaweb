...
<html>
  ...
  <body> <!--/Inicio del body-->  
    ...
    <?php
      ...
      if ($contador = 1){
        //$prname=$_GET['category']; //Recibe de el filtro la categoria seleccionada//
        $contador = 0;
        //echo "---".$_GET['category']."***";//  se utilizo para visualizar el contenido de category ya que category es el nombre del campo de la tabla category //
        //echo "---".$prname."***"; //Esta variable almacena el nombre de la categoria que envia el boton filtrar categorias
      }
    ?>
      ...
      <form action="products_admin.php" target="_self"> <!-- Envia utilizando get por defecto la categoria seleccionada por el usuario para filtrar y Fuerza que el documento referenciado por el enlace sea mostrado en la misma ventana donde esta el enlace -->
        <p>
...
if(isset($_GET['category'])!="") {
  ...
  //echo "---".$buscafiltro."***"; visualiza una parte de codigo de sql el cual cuando no se ejecuta el boton filtrar categorias el contenido es vacio y muestra todos los productos sin filtrar ya que el if anterior pregunta si la variable category es diferente de vacio y si se cumple la variable buscafiltro almacena un codigo de sql para unirlo como condicion de que pro.forcategoria sea igual a $prname
  //echo "Productos Filtrados por la Categoria = ".$_GET['category']."";
